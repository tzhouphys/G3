...
def calculate_amplitude(args):
    ...
    for i in range(Ngrid*Ngrid):
        l = l_list[i]
        radiation_amplitude_pk += (radiation_amplitude(p1, k, l)+radiation_amplitude(p2, k, l))*asymmetry_variable(k, l, z)
    ...
    return [radiation_amplitude_pk/(Ngrid*Ngrid), eta_phi(k)[0]]
...
for CHN in range(1, 10):
    ...
    if __name__ == "__main__":
        ...
        # save the data
        np.save(f'data/500k_rot/amp_list_real_500k_k1_eta_5{CHN}.npy', amp_list_combined)
        np.save(f'data/500k_rot/k_list_real_500k_k1_eta_5{CHN}.npy', k_list)
