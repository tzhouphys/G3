...
def calculate_amplitude(args):
    ...
    return np.array([radiation_amplitude_pk/Nevents, radiation_amplitude_kk/Nevents, radiation_amplitude_combined/Nevents])
...
if __name__ == "__main__":
    ...
    # Collect results
    amp_list_pk = np.array([res[0] for res in results]).reshape(len(phi_list), len(eta_list)).T
    amp_list_k1k2 = np.array([res[1] for res in results]).reshape(len(phi_list), len(eta_list)).T
    amp_list_combined = np.array([res[2] for res in results]).reshape(len(phi_list), len(eta_list)).T
    # save the data
    np.save('amp_list_pk.npy', amp_list_pk)
    np.save('amp_list_k1k2.npy', amp_list_k1k2)
    np.save('amp_list_pk_kk_combined.npy', amp_list_combined)
